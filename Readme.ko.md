...
| **A** | 좌측 이동 |
| **D** | 우측 이동 |
| **Space** | 점프 (게이지 방식, [플레이어 시스템](#플레이어-시스템-hp--mp--armor) 참고) |
...
| **L** | 원거리 **레인(Rain)** 공격 (HP 300 초과, MP 200 초과일 때 사용 가능) |
...
- **MP** (`playerMP_width`, 최대 360): 레인 공격(`L`)을 사용하지 않는 동안 프레임당 +0.05씩 자동 회복되며, 알코올·레인·백신 스킬 사용 시 소모됩니다.
...
- **점프 게이지** (`Jplayer_width`, 스테이지 2 전용): 시간이 지나면 최대 100까지 차오르고, `Space`를 누르고 있는 동안 프레임당 4씩 소모되며 플레이어를 위로 밀어 올립니다. 공중에 떠 있는 상태에서도 게이지가 재충전될 수 있기 때문에, `Space`를 계속 누르고 있으면 원래 의도보다 더 오래 상승할 수 있습니다(팀에서도 의도치 않은 "치트"로 문서화한 부분입니다 — [알려진 이슈](#알려진-이슈--회고) 참고).
...
**플레이어 공격:** 근접 소드/캔디 공격(`J` + 방향키, Armor 상태에 따라 모션이 달라짐)과 알코올 투사체 공격(`K` + 방향키, MP 소모). **원거리 레인 공격**(`L`)은 HP 300 초과, MP 200 초과일 때 사용 가능하며, 3종류의 층 몬스터 모두에게 데미지를 줍니다.
...
- 원거리 공격은 원래 계획했던 만큼 완전하게 구현되지는 못했습니다.
